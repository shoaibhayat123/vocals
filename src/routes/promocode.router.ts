...
// controllers
import promocodeController, { PromoCodeController } from '../controllers/promocode.controller';
// model or interfaces
...
export class PromoCodeRouter {
    ...
    private async get(req, res) {
        try {
            ...
            const contacts = await this.promocodeController.get(search, type);
    ...
    private async getBy(req, res) {
        try {
            ...
            const contact = await this.promocodeController.getBy(search);
    ...
    private async create(req, res) {
        try {
            const result = await this.promocodeController.create({ payload: req.body });
            res.json(result);
        } catch (error:any) {
            console.log({error});
            res.status(error.status || 500).send(!error.status ? new InternalServerError("Something wrong") : error);
    ...
    private async edit(req, res) {
        try {
            ...
            const result = await this.promocodeController.edit({  query: { id: search },payload: req.body });
            res.json(result);
        } catch (error:any) {
            console.log({error});
            res.status(error.status || 500).send(!error.status ? new InternalServerError("Something wrong") : error);
    ...
    private async delete(req, res) {
        try {
            ...
            const result = await this.promocodeController.delete({ query: query });
            res.json(result);
        } catch (error:any) {
            res.status(error.status || 500).send(!error.status ? new InternalServerError("Something wrong") : error);
        }
...
export default new PromoCodeRouter(promocodeController);