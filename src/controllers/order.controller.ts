                ...
                order: [
                    { $match: query },
                    {
                        $lookup: {
                            ...
                            let: { order_id: '$_id' },
                            pipeline: [
                                { $addFields: { "order_id": { "$toObjectId": "$order_id" } } },
                                {
                                    $match: {
                                        $expr: { $and: [{ $eq: ["$order_id", "$$order_id"] }, { $ne: ["$deleted", true] }] }
                                    }
                                }]
                        }
                    },
                    ...
                    { $match: _query },
                    {
                        $lookup: {
                            from: 'users',
                            as: 'rider',
                            let: { rider_id: '$delivery.rider_id' },
                            pipeline: [
                                { $addFields: { "rider_id": { "$toObjectId": "$_id" } } },
                                {
                                    $match: {
                                        $expr: { $and: [{ $eq: ["$rider_id", "$$rider_id"] }, { $ne: ["$deleted", true] }] }
                                    }
                                }]
                        }
                    },
                    { $unwind: { path: '$rider', preserveNullAndEmptyArrays: true } },
                    { $project: { "rider.passwordResetToken": 0, "rider.passwordUpdated": 0, "rider.password": 0 } },
                    {
                        $lookup: {
                            from: 'users',
                            as: 'customer',
                            ...
                            pipeline: [
                                { $addFields: { "customer_id": { "$toObjectId": "$_id" } } },
                                {
                                    $match: {
                                        $expr: { $and: [{ $eq: ["$customer_id", "$$customer_id"] }, { $ne: ["$deleted", true] }] }
                                    }
                                }]
                        }
                    },
                    { $unwind: { path: '$customer', preserveNullAndEmptyArrays: true } },
                    { $project: { "customer.passwordResetToken": 0, "customer.passwordUpdated": 0, "customer.password": 0 } },
                    {
                        $lookup: {
                            from: 'restaurants',
                            as: 'restaurant',
                            ...
                            pipeline: [
                                { $addFields: { "restaurant_id": { "$toObjectId": "$_id" } } },
                                {
                                    $match: {
                                        $expr: { $and: [{ $eq: ["$restaurant_id", "$$restaurant_id"] }, { $ne: ["$deleted", true] }] }
                                    }
                                }]
                        }
                    },
                    { $unwind: { path: '$restaurant', preserveNullAndEmptyArrays: true } },
                    {
                        $lookup: {
                            from: 'users',
                            let: { 'rid': '$restaurant.riders' },
                            pipeline: [
                                { $match: { $expr: { $and: [{ $in: ['$_id', { $ifNull: ['$$rid', []] }] }, { $ne: ["$deleted", true] }] } } }
                            ],
                            as: 'riders'
                        }
                    },
                    { $project: { "riders.passwordResetToken": 0, "riders.passwordUpdated": 0, "riders.password": 0 } }
                ]
            }
        },
        {
